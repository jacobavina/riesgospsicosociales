...
tabla2_1 <- planteles_centro_plus %>% 
  filter(Tristeza) %>% 
  group_by(Tristeza, Sexo,  Alcohol) %>% 
  summarise(n = n()) %>% 
  group_by(Tristeza, .drop = TRUE) %>% 
  mutate(pct = n / sum(n) * 100)

tabla2_1 %>% 
  kable("pipe", caption = "Cuando han sentido tristeza")

# relación entre tristeza, alcohol y sexo cuando no han sentido tristeza
tabla2_2 <- planteles_centro_plus %>% 
  filter(!Tristeza) %>% 
  group_by(Tristeza, Sexo,  Alcohol) %>% 
  summarise(n = n()) %>% 
  group_by(Tristeza, .drop = TRUE) %>% 
...
tabla2_2 %>% 
  kable("pipe", caption = "Cuando no han sentido tristeza")
```
Para este caso el tamaño de nuestra muestra es demasiado pequeño por lo que podríamos caer en conclusiones anticipadas. Lo que nos dicen los datos es que del grupo de problación que no se identifica como hombre o mujer el `r tabla2_1 %>% filter(Tristeza, !Alcohol, Sexo) %>% data.frame() %>% select(pct) %>% unlist() %>% round(digits = 2)` por ciento ha sentido `tristeza`. Mientras que solo el `r tabla2_1 %>% filter(Tristeza, Alcohol, Sexo) %>% data.frame() %>% select(pct) %>% unlist() %>% round(digits = 2)` por ciento ha sentido tristeza y ha consumido alcohol para emborracharse.

De los alumnos que tomaron alcohol para emborracharse ninguno dijo haber sentido `tristeza`. 
